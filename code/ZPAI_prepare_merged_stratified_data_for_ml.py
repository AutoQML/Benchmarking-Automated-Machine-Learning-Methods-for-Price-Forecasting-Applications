...
def prepare_merged_data_for_ml(machine_model, machine_type, file_path_pics, file_path_data, input_filename, summery_file, pca_num, random_state, m_date):
    ...
    auto_index = ['Test-MWE', 'Test-RMSE', 'Test-R2', 'Duration']
    # create result data frame
    ...
    # Split the data
    X_train_preprep, X_test_preprep = train_test_split(machine_type, test_size=0.1, random_state=RANDOM_STATE, stratify=machine_type['model'])
    ...
    X_train_preprep_cat = pd.get_dummies(X_train_preprep, columns=categorical_cols) # Preprocess categorical attributes
    X_test_preprep_cat = pd.get_dummies(X_test_preprep, columns=categorical_cols) # Preprocess categorical attributes

    # confert all numerial values to type int
    X_train_preprep = X_train_preprep_cat.astype(int)
    X_test_preprep = X_test_preprep_cat.astype(int)

    # calculate the number of columns without the price column (therfore  -1)
    column_count = len(X_train_preprep.columns) -1
    ...
    col_list = list(set().union(machine_type_X_train.columns, machine_type_X_test.columns))

    machine_type_X_train = machine_type_X_train.reindex(columns=col_list, fill_value=0)
    machine_type_X_test = machine_type_X_test.reindex(columns=col_list, fill_value=0)
    ...
    ####################
    # machine_type_X_test = machine_type_X_test.reindex(labels=machine_type_X_train.columns,axis=1)
    # machine_type_X_test = machine_type_X_test.reindex(columns=col_list, fill_value=0)
    # machine_type_X_test = machine_type_X_test.fillna(0)
